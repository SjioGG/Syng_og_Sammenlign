    ...
    formattedText("")
{
    ...
    loadLyricsFromFile("/home/stud/Documents/NyBASGUI/NyGUI/NYGUI/SOSr/SOS/ghost-jb.txt");
...
void Song1::startLyricsDisplay() {
    ...
    if (!lyricsTimer) {
        ...
        if (!soundEffect) {
            soundEffect = new QSound("/home/stud/Documents/NyBASGUI/NyGUI/NYGUI/SOSr/SOS/JB-ghost.wav", this);
            soundEffect->play();
        }
}
...
void Song1::updateLyrics() {
    ...
    if (lineCounter < ghostLyrics.size()) {
        highlightCurrentLine(ghostLyrics.at(lineCounter));
        lineCounter++;
    } else {
        lyricsTimer->stop();
        qDebug() << "Lyrics display stopped.";
    }*/
    qDebug() << "Updating lyrics, lineCounter:" << lineCounter;
        ...
        if (lineCounter < ghostLyrics.size()) {
            ...
            int delayTime = 1000000; // Adjust the delay time as needed
            if (lyricsTimer) {
                lyricsTimer->start(delayTime);
            }
        } else {
...
void Song1::on_pushButton_clicked() {
    // Implement the logic to be executed when the first button is clicked
    if (!songlist) {
